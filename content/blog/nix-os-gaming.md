...
移動したら、nanoなどのエディターで`configuration.nix`を開いてファイルの一番下にある`}`の中にこれを追加します。
...
このコマンドの実行結果からFAT32でフォーマットされている先ほど拡張したbootボリュームと、NixOSをインストールしたボリュームを見つけます。僕の場合NixOSが入ったボリュームが`nvme0n1p5`
...
ここまで長旅お疲れ様でした。これでNixOS + Windowsで快適異世界(systemd-boot)ゲーミングライフを送れますね！クリスマスはぼっちでゲームしましょう！
GG
